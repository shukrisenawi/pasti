<x-app-layout>
    ...
    <section class="card space-y-4">
        ...
        <form method="POST" action="{{ route('n8n-settings.update') }}" class="space-y-4">
            ...
            <div>
                <label for="text_program_response_reminder" class="label-base">Teks Group Guru - Minta Respond Program <span class="text-rose-600">*</span></label>
                <textarea id="text_program_response_reminder" name="text_program_response_reminder" rows="4" required class="input-base">{{ old('text_program_response_reminder', $settings['text_program_response_reminder'] ?? '') }}</textarea>
                <p class="mt-1 text-xs text-slate-500">Placeholder: <code>{program_title}</code>, <code>{senarai_guru}</code></p>
            </div>
