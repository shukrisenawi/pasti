<x-app-layout>
    ...
    <div class="grid gap-4 md:grid-cols-4">
        ...
        <div class="stat-card">
            ...
            <p class="stat-value">{{ number_format((float) ($guru->kpiSnapshot?->score ?? 0), 0) }}</p>
        </div>
    </div>

    <div class="table-wrap mt-4">
        <table class="table-base">
            <thead><tr><th>{{ __('messages.title') }}</th><th>{{ __('messages.markah') }}</th><th>{{ __('messages.date') }}</th><th>{{ __('messages.location') }}</th><th>{{ __('messages.status') }}</th></tr></thead>
