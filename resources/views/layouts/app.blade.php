    ...
    >
        ...
        <nav class="space-y-1 px-3 py-2 text-sm">
            ...
            @if($isAdminCardUser)
                ...
                <div x-data="{ open: {{ request()->routeIs(['financial.*', 'claims.*', 'kpi.gurus.*', 'users.expired-skim-pas', 'pemarkahan.*', 'pasti-information.*', 'guru-salary-information.*', 'shirt-purchases.*', 'pasti-reports.*', 'kursus-guru.*', 'programs.*']) ? 'true' : 'false' }} }" class="space-y-1">
                    <button @click="open = !open" class="menu-link w-full flex items-center justify-between {{ request()->routeIs(['financial.*', 'claims.*', 'kpi.gurus.*', 'users.expired-skim-pas', 'pemarkahan.*', 'pasti-information.*', 'guru-salary-information.*', 'shirt-purchases.*', 'pasti-reports.*', 'kursus-guru.*', 'programs.*']) ? 'text-primary bg-primary/5' : '' }}">
                        <div class="flex items-center gap-2">
            ...
            @if($isGuruOnly)
                    <div x-data="{ open: {{ request()->routeIs(['kpi.guru.show', 'claims.*', 'pemarkahan.*', 'pasti-information.*', 'guru-salary-information.*', 'shirt-purchases.*', 'kursus-guru.*', 'programs.*']) ? 'true' : 'false' }} }" class="space-y-1">
                        <button @click="open = !open" class="menu-link w-full flex items-center justify-between {{ request()->routeIs(['kpi.guru.show', 'claims.*', 'pemarkahan.*', 'pasti-information.*', 'guru-salary-information.*', 'shirt-purchases.*', 'kursus-guru.*', 'programs.*']) ? 'text-primary bg-primary/5' : '' }}">
                            <div class="flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 2v-6m-8-2h12a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h4l2 2z" /></svg>
